...
Module NativeMethods
    ...
    Public Function GetFormattedDeviceID() As String
        If _formattedDeviceID Is Nothing Then
            If _deviceId Is Nothing Then GetDeviceID()
            If String.IsNullOrEmpty(_deviceId) Then Return Terminology.GetString(My.Resources.SharedMessagesKey, RES_UnsupportedDeviceId)
            Dim sb As New StringBuilder(100)
            If _deviceId.Length > 6 Then
                sb.Append(_deviceId.Substring(0, 6))
                If _deviceId.Length >= 12 Then
                    sb.Append(My.Resources.DeviceIdSeparator)
                    sb.Append(_deviceId.Substring(6, 6))
                    If _deviceId.Length > 12 Then
                        Dim i As Integer
                        For i = 12 To _deviceId.Length - 1 Step 5
                            Dim part As String
                            If _deviceId.Length - i >= 5 Then
                                part = _deviceId.Substring(i, 5)
                            Else
                                part = _deviceId.Substring(i)
                            End If
            ...
            Else
                sb.Append(_deviceId)
            End If
            _formattedDeviceID = sb.ToString()
            '_formattedDeviceID = String.Format(WithoutCulture, "{0}.{1}.{2}.{3} . {4}.{5}.{6}.{7}", _
            '    _deviceId.Substring(0, 4), _
            '    _deviceId.Substring(4, 4), _
            '    _deviceId.Substring(8, 4), _
            '    _deviceId.Substring(12, 4), _
            '    _deviceId.Substring(16, 4), _
            '    _deviceId.Substring(20, 4), _
            '    _deviceId.Substring(24, 4), _
            '    _deviceId.Substring(28, 4))
        End If
        Return _formattedDeviceID
    End Function
