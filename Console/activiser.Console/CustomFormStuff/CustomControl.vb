...
Partial Public Class CustomControl
    Private Shared RunTimeScale As Double = 1 'MyDpiX / DesignDpi
    ...
    Private _customControlRow As FormDefinition.FormFieldRow
    ...
    Private WithEvents _consoleData As ConsoleData

    'HACK: pseudo-index for ListBox; because we reset the 'selectedIndex' to -1 after setting the 'SelectedValue', in order to make it look pretty.
    Private _listBoxIndex As Integer = -1

#Region "Control additions"

    Private Sub addCheckBox(ByVal c As FormDefinition.FormFieldRow)
    ...
    Private Sub addListBox(ByVal c As FormDefinition.FormFieldRow)
        ...
        If (c.LockCode = LockCodes.Locked) Then ' load the 'readonly' list box; we'll only be showing one line.
            ...
            Me.ListBox1.BorderStyle = Windows.Forms.BorderStyle.Fixed3D
    ...
    Private Sub addDatePicker(ByVal c As FormDefinition.FormFieldRow)
    ...
    Private Sub addDateTimePicker(ByVal c As FormDefinition.FormFieldRow)
        Me.DateTimePicker1 = New System.Windows.Forms.DateTimePicker
        Me.DateTimePicker1.Font = _font
        Me.DateTimePicker1.Dock = System.Windows.Forms.DockStyle.Top
        Me.DateTimePicker1.Height = Me.dummyCombo.Height
    ...
    Private Sub addNumberBox(ByVal c As FormDefinition.FormFieldRow)
        Me.NumericUpDown1 = New System.Windows.Forms.NumericUpDown
        Me.NumericUpDown1.Font = _font
        Me.NumericUpDown1.BorderStyle = System.Windows.Forms.BorderStyle.Fixed3D
        ...
        Me.NumericUpDown1.Maximum = c.MaximumValue
        Me.NumericUpDown1.Minimum = c.MinimumValue
    ...
    Private Sub addTextBox(ByVal c As FormDefinition.FormFieldRow)
        Me.TextBox1 = New System.Windows.Forms.TextBox
        Me.TextBox1.Name = c.AttributeName & "TextBox"
        Me.TextBox1.BorderStyle = Windows.Forms.BorderStyle.Fixed3D
    ...
    Private Sub addTimePicker(ByVal c As FormDefinition.FormFieldRow)
        Me.DateTimePicker1 = New System.Windows.Forms.DateTimePicker
        Me.DateTimePicker1.Font = _font
        Me.DateTimePicker1.Dock = System.Windows.Forms.DockStyle.Top
        Me.DateTimePicker1.Height = Me.dummyCombo.Height
...
#End Region
    ...
    Sub New(ByVal controlRow As FormDefinition.FormFieldRow, ByVal table As DataTable, ByVal parent As CustomFormPanel, ByVal font As Font)
            ...
            If Not String.IsNullOrEmpty(_listDataSource) Then
                If Not String.IsNullOrEmpty(_listValueColumn) Then
                    If Not String.IsNullOrEmpty(_listDisplayColumn) Then
                        Dim listSourceTable As DataTable = ConsoleData.CoreDataSet.Tables(_listDataSource)
    ...
    Public ReadOnly Property CustomFormRow() As FormDefinition.FormRow
        Get
            Return _customForm
        End Get
    End Property
    ...
    Public ReadOnly Property CustomControlRow() As FormDefinition.FormFieldRow
        Get
        ...
        If p IsNot Nothing Then
            ...
            If Me.CustomControlRow IsNot Nothing Then
                If Me.CustomControlRow.Position = "F" Then
                    result -= System.Windows.Forms.SystemInformation.VerticalScrollBarWidth
                Else
                    result -= System.Windows.Forms.SystemInformation.VerticalScrollBarWidth \ 2
    ...
    Private Sub PdaControl_BackColorChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.BackColorChanged
        Me.Caption.BackColor = Me.BackColor
    End Sub
            ...
            Select Case Me._controlType
                ...
                Case ControlType.Number
                    If Me._dataType Is GetType(Double) Then
                        Return Decimal.ToDouble(Me.NumericUpDown1.Value)
                    ElseIf Me._dataType Is GetType(Int32) Then
                        Return Decimal.ToInt32(Me.NumericUpDown1.Value)
                    ElseIf Me._dataType Is GetType(Byte) Then
                        Return Decimal.ToByte(Me.NumericUpDown1.Value)
                    ElseIf Me._dataType Is GetType(Int16) Then
                        Return Decimal.ToInt16(Me.NumericUpDown1.Value)
                    ElseIf Me._dataType Is GetType(Int64) Then
                        Return Decimal.ToInt64(Me.NumericUpDown1.Value)
                    ElseIf Me._dataType Is GetType(Single) Then
                        Return Decimal.ToSingle(Me.NumericUpDown1.Value)
...
#Region "Thread safety delegates"
            ...
            ElseIf Control Is Me.ListBox1 Then
                For Each li As ListFiller In Me.ListBox1.Items
                    If li.Key.Equals(Value) Then
                        ...
                        Me._listBoxIndex = Me.ListBox1.SelectedIndex
    ...
    Private Delegate Sub SetNumericUpDownValueCallBack(ByVal Control As System.Windows.Forms.NumericUpDown, ByVal Value As Decimal)
    Private Sub SetNumericUpDownValue(ByVal Control As System.Windows.Forms.NumericUpDown, ByVal Value As Decimal)
        If Control.InvokeRequired Then
...
#End Region
    ...
    Private Sub _consoleData_Refreshed(ByVal sender As Object, ByVal e As ConsoleDataRefreshEventArgs) Handles _consoleData.Refreshed
        If e.Updates Is Nothing Then Return
        If Not Me.ControlType = ControlType.List Then Return

        If String.IsNullOrEmpty(_listDataSource) OrElse String.IsNullOrEmpty(_listValueColumn) OrElse String.IsNullOrEmpty(_listDisplayColumn) Then
            Dim ccr As FormDefinition.FormFieldRow
            If e.Updates.Tables.Contains("CustomControl") Then
                For Each ccr In e.Updates.Tables("CustomControl").Rows
                    If ccr.FormFieldId = _customControlRow.FormFieldId Then
                        If ccr.ListData <> _customControlRow.ListData Then
                            _customControlRow.ListData = ccr.ListData
                            Me.FillList()
                        End If
                    End If
                Next
        ...
        If e.Updates.Tables.Contains(Me.ListDataSource) AndAlso e.Updates.Tables(Me.ListDataSource).Rows.Count <> 0 Then
            Me.FillList() ' reload list.
        End If
    End Sub
End Class
