...
Partial Friend Class Synchronisation
    ...
    Shared Sub New()
        SetSyncInterval(AppConfig.GetSetting(My.Resources.AppConfigSyncIntervalKey, 60))
        SetAutoSyncInterval(AppConfig.GetSetting(My.Resources.AppConfigAutoSyncIntervalKey, 60), AppConfig.GetSetting(My.Resources.AppConfigAutoSyncKey, False))
    End Sub
...
#Region "Sounds"
    ...
    Friend Shared Sub initSounds()
        ...
        If _syncStartSound Is Nothing AndAlso AppConfig.GetSetting("PlaySyncStartSound", True) Then
            Try : _syncStartSound = New SoundPlayer(IO.Path.Combine(gDatabaseRoot, My.Resources.SoundsSyncStarted)) : Catch : End Try
        End If

        If _syncCompleteSound Is Nothing AndAlso AppConfig.GetSetting("PlaySyncCompleteSound", True) Then
            Try : _syncCompleteSound = New SoundPlayer(IO.Path.Combine(gDatabaseRoot, My.Resources.SoundsSyncComplete)) : Catch : End Try
        End If

        If _syncNewRequestSound Is Nothing AndAlso AppConfig.GetSetting("PlayNewRequestSound", True) Then
