...
{
    public class RouteEntity : IPlugin, IDisposable
    {
        private activiserInputGatewayCrm.activiserInputGatewayCrm crmIg;

        public RouteEntity(string config, string secureConfig)
        {
        ...
        public void Execute(IPluginExecutionContext context)
        {
            if (crmIg != null)
            {
                try
                {
                    string entityIdentity = Serialization.Serialize((Moniker)context.InputParameters[ParameterName.EntityMoniker]);
                    string newAssignee = Serialization.Serialize((DynamicEntity)context.InputParameters[ParameterName.Assignee]);
                    ...
                    crmIg.Route(txId, context.OrganizationId.ToString(Properties.Resources.OrganizationIdFormat), context.UserId.ToString(), context.PrimaryEntityName, entityIdentity, newAssignee, preData, postData);
                }
                catch (Exception ex)
                {
                    Logging.LogError(ErrorCode.RouteEntityException, Properties.Resources.RouteEntityExceptionMessage,
