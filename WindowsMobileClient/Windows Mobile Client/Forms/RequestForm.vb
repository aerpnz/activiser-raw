...
#End If
        ...
        Try
            ...
            If Me.ClientListComboBox.SelectedIndex <> -1 Then ' ClientListComboBox.DataSource IsNot Nothing AndAlso Me.ClientListComboBox.SelectedIndex <> -1 Then
                If _SelectedClient Is Nothing Then
                    Dim listItem As SelectorItem(Of Guid) = TryCast(Me.ClientListComboBox.SelectedItem, SelectorItem(Of Guid))
                    If listItem IsNot Nothing Then
                        _SelectedClient = gClientDataSet.ClientSite.FindByClientSiteUID(listItem.Value)
                    End If
                End If
    ...
    Private Sub PopulateClientList()
        ...
        Try
            Dim clientList As New Generic.SortedList(Of String, SelectorItem(Of Guid))
            For Each csr As ClientSiteRow In gClientDataSet.ClientSite
                If csr.ClientSiteStatusRow.IsActive AndAlso Not csr.IsSiteNameNull AndAlso Not String.IsNullOrEmpty(csr.SiteName) Then
                    If gAllowRequestsForClientsOnHold OrElse Not csr.Hold Then
                        clientList.Add(csr.SiteName, New SelectorItem(Of Guid)(csr.ClientSiteUID, csr.SiteName))
                    End If
                End If
            Next
            ...
            For Each cli As SelectorItem(Of Guid) In clientList.Values
...
#Region "Custom Form Support"
    ...
    Private Sub LoadCustomForms()
        ...
        For Each cfr As WebService.FormDefinition.FormRow In requestForms.Values
            Dim tp As New TabPage
            tp.Text = cfr.FormLabel
            tp.Name = cfr.FormName
            Me.TabControl.TabPages.Insert(Me.TabControl.TabPages.Count - 2, tp)
...
#End If
        ...
        Dim mainFont As Font = New Font(Me.Font.Name, AppConfig.GetSetting(My.Resources.AppConfigTextSizeKey, 8), FontStyle.Regular)
        Dim detailFont As New Font(Me.Font.Name, AppConfig.GetSetting(My.Resources.AppConfigTextSizeKey, 8) - 1, FontStyle.Regular)
...
#Region "Input Panel Support"
    ...
    Private Sub EnableInputPanelOnFormLoad(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        EnableToolbars(AppConfig.GetSetting(My.Resources.AppConfigRequestShowToolBarsKey, Me.Height > Me.Width))
    End Sub
...
#Region "Toolbar Support"
    ...
    Private Sub EnableToolbars(ByVal state As Boolean)
        ...
        AppConfig.SaveSetting(My.Resources.AppConfigRequestShowToolBarsKey, state)
    End Sub
