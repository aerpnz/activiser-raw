...
--DELETE FROM <activiser DB, sysname, activiser>.<activiser DB owner, sysname, dbo>.[GatewayException]
...
--
-- Populate ClientSite Status; for Microsoft CRM, this is either Active or Inactive
--
...
SELECT     AttributeValue, Value, CASE WHEN [Value] LIKE '%Inactive%' THEN 0 ELSE 1 END AS Expr1
FROM         <Microsoft_CRM_Database_Name, sysname, >.<CRM DB owner, sysname, dbo>.StringMap
WHERE     (ObjectTypeCode = 1) AND (AttributeName = 'statecode')
GO

--
-- Populate Request Status; for Microsoft CRM, this is the 'incidentstagecode' attribute of the 'incident' (case) entity.
-- 
...
-- 
-- Add Consultants; for CRM, these are the 'system users'
--
...
SELECT		SystemUserId, FullName, row_number() over(order by fullname), 0 AS MinSyncTime, MobilePhone, 
...
FROM        <Microsoft_CRM_Database_Name, sysname, >.<CRM DB owner, sysname, dbo>.SystemUserBase
...
-- 
-- Add ClientSites; for CRM, these are 'Accounts'
--
-- these are done one at a time to allow the underlying trigger to set the correct client site number
...
INSERT INTO <activiser DB, sysname, activiser>.<activiser DB owner, sysname, dbo>.ClientSite
                      ...
                      ContactPhone2, CreatedDateTime, M odifiedDateTime)
...
FROM         <Microsoft_CRM_Database_Name, sysname, >.<CRM DB owner, sysname, dbo>Account
...
-- 
-- Add Requests; for CRM, these are 'Cases', but the underlying Database entity is an 'incident'
--
...
FROM        <Microsoft_CRM_Database_Name, sysname, >.<CRM DB owner, sysname, dbo>.Incident
...
-- 
-- Add Jobs; for CRM, these are the 'service activities', with an underlying database entity of 'serviceappointment'
--
...
FROM		<Microsoft_CRM_Database_Name, sysname, >.<CRM DB owner, sysname, dbo>.ServiceAppointment
...
FROM      <activiser DB, sysname, activiser>.<activiser DB owner, sysname, dbo>.Job INNER JOIN
                      <activiser DB, sysname, activiser>.<activiser DB owner, sysname, dbo>.Request ON <activiser DB, sysname, activiser>.<activiser DB owner, sysname, dbo>.Job.RequestUID = <activiser DB, sysname, activiser>.<activiser DB owner, sysname, dbo>.Request.RequestUID
GO
